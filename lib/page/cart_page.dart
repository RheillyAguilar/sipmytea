...
Future<void> _handleInventory(CartItem item) async {
  ...
  final deductions = <String, int>{
    ...
    if (name.contains('fries')) 'fries': 150,
    if (name.contains('combo')) 'fries': 50,
    ...
    if(name.contains('cheese beef')) 'slice cheese' : 2,
    if (name.contains('combo')) 'slice cheese': 1,
    if (validSize) ..._getSmoothieDeductions(category, name, size),
    if (validSize) ..._getFreshTeaDeduction(category, name, size),
    if (validSize) ..._getCreampuffDeduction(category, name, size),
    if (validSize) ..._getClassicDeduction(category, name, size),
    ..._deductAddons(item.addOns)
  };
  ...
  // Handle cups - only if size is valid
  if (validSize) {
    ...
    // Handle straw - only if size is valid
